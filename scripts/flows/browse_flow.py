#!/usr/bin/env python3
"""
Browse Flow - Extracted from auto_browse_cam_v3.py

Provides reusable browser capture functionality with state tracking.
"""
...
def detect_state_change(page) -> Dict[str, Any]:
    ...
    try:
        ...
        # 提取可见下拉框
        try:
            dropdowns = page.locator("select:visible, [role='combobox']:visible").all()
...
def capture_screenshot_with_state_tracking(
    page,
    recorder,
    ...
    # 检测当前状态
    current_state = detect_state_change(page)

    # 检查是否为重复状态
    fingerprint = state_tracker.generate_fingerprint(current_state)
    if state_tracker.has_visited_state(fingerprint):
        logger.info(f"Skipping duplicate state: {step_name}")
        return None

    # 记录新状态
    state_tracker.mark_state_visited(fingerprint)
    ...
    try:
        selects = page.locator("select:visible, [role='combobox']:visible").all()
        page_info["dropdowns"] = [
            {
...
def capture_interactive_elements(page, recorder, state_tracker: StateTracker, selector_engine: SelectorEngine):
            ...
            for idx, btn in enumerate(new_buttons[:1]):  # Only click first one
                try:
                    if btn.is_visible():
                        ...
                        capture_screenshot_with_state_tracking(page, recorder, state_tracker, step_name, description)

                        # Try to capture form fields in the dialog
                        capture_dialog_fields(page, recorder, state_tracker)

                        # Close dialog if there's a close button
                        try:
                            close_button_selectors = selector_engine.get_fallback_selectors(
                                element_type="button",
                                aria_label="Cancel",
                                text="Cancel"
                            )
                            for selector in close_button_selectors:
                                try:
                                    close_btns = page.locator(f"{selector}:visible").all()
                                    if close_btns:
                ...
                for idx, element in enumerate(elements[:3]):  # Limit to first 3
                    try:
                        ...
                        if "menu" in element_type:
                            explore_menu_items(page, recorder, state_tracker, selector_engine, element_type, idx)
...
def explore_menu_items(page, recorder, state_tracker: StateTracker, selector_engine: SelectorEngine, menu_type: str, menu_idx: int):
    ...
    try:
        ...
        for item_idx, item in enumerate(menu_items[:5]):  # Limit to first 5 items
            try:
                ...
                try:
                    dialog_visible = page.locator("[role='dialog']:visible, .v-dialog--active:visible").count() > 0
                    if dialog_visible:
                        ...
                        for selector in close_button_selectors:
                            try:
                                close_buttons = page.locator(f"{selector}:visible").all()
                                if close_buttons:
                ...
                try:
                    if item_idx < len(menu_items) - 1:  # Not the last item
                        ...
                        for btn_selector in menu_button_selectors:
                            try:
                                btns = page.locator(btn_selector).all()
...
def capture_modals(page, recorder, state_tracker: StateTracker):
    ...
    for selector in modal_selectors:
        try:
            modals = page.locator(selector).all()
            if modals:
...
def capture_dialog_fields(page, recorder, state_tracker: StateTracker):
    ...
    try:
        ...
        for selector in dialog_selectors:
            try:
                dialogs = page.locator(f"{selector}:visible").all()
...
def automatic_capture_flow(page, recorder, target_url: str):
    """Automatically explore and capture the page"""
    logger.info("=" * 60)
    logger.info("AUTOMATIC CAPTURE MODE - V3 (STATE TRACKING)")
    logger.info("=" * 60)

    # Initialize state tracker and selector engine
    state_tracker = StateTracker()
    ...
    capture_screenshot_with_state_tracking(page, recorder, state_tracker, "01-initial-page", "Initial page load")

    # Step 2: Find and capture interactive elements
    capture_interactive_elements(page, recorder, state_tracker, selector_engine)

    # Step 3: Capture any modals or dialogs
    capture_modals(page, recorder, state_tracker)

    logger.info("=" * 60)
    logger.info(f"CAPTURE COMPLETE - {recorder.get_step_count()} steps captured")
    logger.info("=" * 60)

    return recorder.captured_steps
